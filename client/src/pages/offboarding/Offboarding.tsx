...
import { Plus, CheckCircle, Clock, AlertCircle, X, UserMinus, Edit2, Save, Calendar } from 'lucide-react';
import toast from 'react-hot-toast';
import api from '../../lib/api';
import { formatDate } from '../../lib/utils';
import type { OffboardingProcess, User } from '../../types';
...
export default function Offboarding() {
  ...
  const handleCreateProcess = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedUserId || !lastWorkingDay) return;
    ...
    try {
      ...
      toast.error(error.response?.data?.error || 'Failed to create offboarding process');
    }
  };
  ...
  const handleToggleChecklistItem = async (processId: string, checklist: ChecklistItem[], itemId: number) => {
    try {
      ...
      await api.put(`/offboarding/${processId}`, { 
  ...
  const handleSaveNotes = async (processId: string) => {
    try {
      await api.put(`/offboarding/${processId}`, { notes: editNotes });
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    try {
      await api.put(`/offboarding/${id}`, { status });
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage employee offboarding processes</p>
        </div>
        <button
      ...
      {/* Create Form */}
      {showCreateForm && (
        <form onSubmit={handleCreateProcess} className="bg-red-50 border border-red-200 rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
      ...
      <div className="grid grid-cols-1 gap-6">
        {processes.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            <p className="text-gray-500">No offboarding processes. Click "New Offboarding" to get started.</p>
          </div>
        ) : (
            ...
            return (
              <div key={process.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4 py-3 border-y">
                  ...
                  {process.lastWorkingDay && (
                    <div className="text-sm">
                      <span className="text-gray-600">Last Day:</span>
                      <p className="font-medium text-red-600">{formatDate(process.lastWorkingDay)}</p>
                    </div>
                  )}
                  {process.reason && (
                    <div className="text-sm">
                      <span className="text-gray-600">Reason:</span>
                      <p className="font-medium text-gray-900">{process.reason}</p>
                    </div>
                  )}
                ...
                <div className="space-y-2">
                  <h4 className="font-medium text-gray-900 text-sm">Offboarding Checklist:</h4>
                  ...
                  {editingProcess === process.id ? (
                    <div className="space-y-2">
                      <textarea
                        ...
                        rows={3}
                        placeholder="Add notes about offboarding progress..."
                      />
